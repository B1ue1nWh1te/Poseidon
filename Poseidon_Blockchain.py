...
class Chain():
    ...
    def GetStorage(self, Address: str, Index: int) -> str:
        Data = hex(bytes_to_long(self.Net.eth.get_storage_at(Address, Index)))
    ...
    def DumpStorage(self, Address: str, Count: int) -> list:
        ...
        Temp = '\n'.join(Data)
        logger.info(f"\n[DumpStorage][{Address}][0~{Count-1}]\n{Temp}")
        return Data


class Account():
    def __init__(self, Chain: Chain, PrivateKey: int) -> None:
        self.Chain = Chain
        self.Net = Chain.Net
    ...
    def SendTransactionToChain(self, To: str, Data: str, Value=0, Gas=3000000) -> dict:
        ...
        TransactionReceipt = self.Net.eth.wait_for_transaction_receipt(TransactionHash)
    ...
    def DeployContractToChain(self, Abi: dict, Bytecode: str, Value=0) -> str:
        ...
        logger.info(f"\n[DeployContract]\n[Abi]{Abi}\n[Bytecode]{Bytecode}")
        TransactionData = Contract.constructor().buildTransaction()
        Txn = {
            "from": self.Address,
            "gasPrice": self.Net.eth.gas_price,
...
def SolidityToAbiAndBytecode(Course: str, ContractName: str) -> tuple:
    CompiledSol = compile_source(open(Course).read())
    ContractData = CompiledSol[f'<stdin>:{ContractName}']
    Abi = ContractData['abi']
    Bytecode = ContractData['bin']
    logger.info(f"\n[SolidityToAbiAndBytecode]\n[Abi]{Abi}\n[Bytecode]{Bytecode}")
    json.dump((Abi, Bytecode), open(f'{ContractName}.json', 'w'))
    return (Abi, Bytecode)
